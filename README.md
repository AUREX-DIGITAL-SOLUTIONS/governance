# Aurex Digital Solutions

## About

Aurex Digital Solutions is a founder-led institution being developed to research, develop, validate, and steward open digital energy coordination infrastructure for African energy systems, beginning in Kenya.

Its near-term focus is facility-level energy management research and prototyping. Its long-term vision is more reliable, resilient, affordable, transparent, flexible, and interoperable digital energy systems across Africa.

This README is the navigation and status gateway for Aurex's institutional documentation. It links only to documents that currently exist.

> **Current status:** Aurex is under institutional development. The documentation baseline does not imply incorporation, formal adoption, licensing, regulatory approval, deployment, or operational authority.

**Version tracking:** [Document Register](docs/04-readiness-and-baseline/DOCUMENT-REGISTER.md) is the authoritative index of current document versions and statuses.

**License:** Documentation is licensed under [CC BY 4.0](./LICENSE). No software license has been adopted.

## Documentation

### Institutional Foundation

- [Founding Thesis](docs/01-institutional-foundation/FOUNDING-THESIS.md)
- [Official Problem Statement](docs/01-institutional-foundation/OFFICIAL-PROBLEM-STATEMENT.md)
- [Vision, Mission & Purpose](docs/01-institutional-foundation/VISION-MISSION-PURPOSE.md)
- [Values & Institutional Principles](docs/01-institutional-foundation/VALUES-AND-INSTITUTIONAL-PRINCIPLES.md)
- [Code of Conduct](CODE-OF-CONDUCT.md)
- [Organization Charter](docs/01-institutional-foundation/ORGANIZATION-CHARTER.md)
- [Governance Framework](docs/01-institutional-foundation/GOVERNANCE-FRAMEWORK.md)
- [Organization Structure & Decision Rights](docs/01-institutional-foundation/ORGANIZATION-STRUCTURE-AND-DECISION-RIGHTS.md)

### Technical Governance

- [Open-Source & Licensing Framework](docs/02-technical-governance/OPEN-SOURCE-AND-LICENSING-FRAMEWORK.md)
- [Contributing Framework](docs/02-technical-governance/CONTRIBUTING-FRAMEWORK.md)
- [Security & Responsible Technology Policy](docs/02-technical-governance/SECURITY-AND-RESPONSIBLE-TECHNOLOGY-POLICY.md)
- [Ethics, Sovereignty & Public-Interest Framework](docs/02-technical-governance/ETHICS-SOVEREIGNTY-AND-PUBLIC-INTEREST-FRAMEWORK.md)

### Operations and Development

- [Risk Management Framework](docs/03-operations-and-development/RISK-MANAGEMENT-FRAMEWORK.md)
- [Operating Model](docs/03-operations-and-development/OPERATING-MODEL.md)
- [Organization Roadmap](docs/03-operations-and-development/ORGANIZATION-ROADMAP.md)
- [Funding & Financial Framework](docs/03-operations-and-development/FUNDING-AND-FINANCIAL-FRAMEWORK.md)
- [Partnership & Stakeholder Framework](docs/03-operations-and-development/PARTNERSHIP-AND-STAKEHOLDER-FRAMEWORK.md)
- [Research & Innovation Framework](docs/03-operations-and-development/RESEARCH-AND-INNOVATION-FRAMEWORK.md)
- [Programme Governance Framework](docs/03-operations-and-development/PROGRAMME-GOVERNANCE-FRAMEWORK.md)

### Readiness and Baseline

- [Legal & Institutional Readiness](docs/04-readiness-and-baseline/LEGAL-AND-INSTITUTIONAL-READINESS.md)
- [Organization Documentation Baseline v1.0](docs/04-readiness-and-baseline/ORGANIZATION-DOCUMENTATION-BASELINE-V1.0.md)
- [Document Register](docs/04-readiness-and-baseline/DOCUMENT-REGISTER.md)

## Technical Direction

Aurex's current technical direction is staged research and engineering:

```text
Research
  -> Facility-level problem validation
  -> Facility EMS prototype
  -> Controlled validation
  -> Pilot
  -> Multi-facility evidence
  -> Regional coordination research
```

Regional DERMS and National DERMS are proposed coordination layers, not deployed infrastructure. The Energy Operating System is a long-term architectural and institutional vision.

## Current Baseline

Tasks 1-21 form the **Aurex Institutional Documentation Baseline v1.0** as of 2026-08-29. The foundational documentation sequence is complete; there is no Task 22.

## Next Phase

Execution continues through three tracks:

- **Institution:** Legal and formalization actions as required
- **Research:** Validated facility-level problem research
- **Engineering:** Facility EMS research and prototype environment

Immediate research should establish where facility-level visibility, interoperability, and coordination gaps exist, what current systems already address, and what evidence justifies a Facility EMS capability.
